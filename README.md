...
3. **前端：依赖与启动**
   ...
   npm run start
   ```
   前端默认 http://localhost:3000。若需将前端的 `/api` 请求代理到后端，可配置 `netops-frontend/package.json` 中的 `proxy` 为 `http://localhost:8000`，或使用项目内的代理脚本（见 [INSTALL.md](INSTALL.md)）。

4. **访问**  
   浏览器打开 http://localhost:3000，使用 `create_admin.py` 创建的管理员账号登录（默认如 admin / admin123，以脚本输出为准）。
